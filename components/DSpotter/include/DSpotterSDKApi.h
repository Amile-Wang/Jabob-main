...
// lpszUID(OUT): The output string of UID.
// nLen(IN): The memory size of lpszUID, it must >= 25.
DSPDLL_API INT DSpotter_GetUidString(char *lpszUid, INT nLen);
...
// Engine support Cmd SG reward from v2.2.7, default is zero.
// Purpose: Set Cmd SG Diff. reward
// hDSpotter(IN): a handle of the recognizer
// nCmdIdx(IN): the command ID. It is 0 based
// nReward(IN): SG Diff. reward
// Return: Success or error code
DSPDLL_API INT DSpotter_SetCmdSGDiffReward(HANDLE hDSpotter, INT nCmdIdx, INT nReward);

// Engine support Cmd SG reward from v2.2.7, default is zero.
// Purpose: Get Cmd SG Diff. reward
// hDSpotter(IN): a handle of the recognizer
// nCmdIdx(IN): the command ID. It is 0 based
// *pnErr(IN/Out): error code
// Return: SG Diff. reward
DSPDLL_API INT DSpotter_GetCmdSGDiffReward(HANDLE hDSpotter, INT nCmdIdx, INT *pnErr);

DSPDLL_API INT DSpotter_ConvertToShortModel(const BYTE* lpbyOrgPackModel, BYTE* lpOutShortModel, INT nOutModelSize);

DSPDLL_API INT DSpotter_GetScoreSum(HANDLE hDSpotter, INT* pnScoreSum);
