...
//XXX_pack_WithTxtAndTriAndMapIDAndTree.bin => CYBase.mod/Group_1.mod/.../Group_N.mod/Group_1.txt/.../Group_N.txt/CYTrimap.mod/AllGroup_MapID_pack.bin/CYTree.mod
...
/* Get the memory address of tree model.
 *   hCybModel(in): The handle of CybModel.
 * Return the memory address of group model or NULL if an error occurs. */
const BYTE* CybModelGetTree(HANDLE hCybModel);
...
/* Get the command information.
	...
	*   pnMapID(out): The map ID of the command which is defined in DSMT, could be NULL.
	* Return the UTF8 command text. If it is a garbage command(reward = -100 in DSMT), empty string will return. */
unsigned short* CybModelGetCommandInfoUTF16(HANDLE hCybModel, int nGroupIndex, int nCommandIndex, unsigned short *lpszCommand, int nCommandLen, int *pnMapID);
