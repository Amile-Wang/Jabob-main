...
void Application::CheckNewVersion(Ota& ota) {
    ...
    while (true) {
        ...
        if (!ota.CheckVersion()) {
            // 添加更多调试信息
            ESP_LOGE(TAG, "Failed to check version. URL: %s", ota.GetCheckVersionUrl().c_str());
            retry_count++;
...
void Application::Start() {
    ...
    auto pwm_servo = board.GetPwmServo();
    if (pwm_servo) {
        pwm_servo->Initialize();
        pwm_servo->SetDutyCycle(26);  // 设置占空比
    }
    ...
    display->SetEmotion("cool"); // 或其他你想要的默认表情
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                Schedule([this]() {
                    if (device_state_ == kDeviceStateSpeaking) {
                        ...
                        // 等待音频播放完成再切换状态
                        // 添加延时确保音频播放完毕，根据需要调整延时时间
                        Schedule([this]() {
                            // vTaskDelay(pdMS_TO_TICKS(3000)); // 1秒延时确保播放完毕
...
void Application::SetOfficeLocation(const std::string& location) {
    office_location_ = location;
}

std::string Application::GetOfficeLocation() const {
