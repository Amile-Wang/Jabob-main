...
bool MicroWakeWord::LoadWakeWordModel() {
    ...
    ESP_LOGI(TAG, "Input tensor: type=%d, scale=%.6f, zp=%d, dims=[%d,%d,%d,%d]",
             (int)in->type, in->params.scale, (int)in->params.zero_point,
             in->dims->size > 0 ? (int)in->dims->data[0] : -1,
             in->dims->size > 1 ? (int)in->dims->data[1] : -1,
             in->dims->size > 2 ? (int)in->dims->data[2] : -1,
             in->dims->size > 3 ? (int)in->dims->data[3] : -1);
    ESP_LOGI(TAG, "Output tensor: type=%d, scale=%.6f, zp=%d, dims=[%d,%d,%d,%d]",
             (int)out->type, out->params.scale, (int)out->params.zero_point,
             out->dims->size > 0 ? (int)out->dims->data[0] : -1,
             out->dims->size > 1 ? (int)out->dims->data[1] : -1,
             out->dims->size > 2 ? (int)out->dims->data[2] : -1,
             out->dims->size > 3 ? (int)out->dims->data[3] : -1);
    return true;
}
...
bool MicroWakeWord::RunFrame(const int16_t* window_pcm) {
    ...
    // Debug: log first 5 feature bytes periodically.
    static uint16_t dbg_feat_count = 0;
    if (++dbg_feat_count >= 300) {
        dbg_feat_count = 0;
        ESP_LOGI(TAG, "DBG feat[0..4]=%d %d %d %d %d", feature[0], feature[1], feature[2], feature[3], feature[4]);
    }
    ...
    if (current_stride_step_ >= input_stride_) {
        ...
        if (output->type == kTfLiteInt8) {
            ...
            prob = static_cast<uint8_t>(std::max(0, std::min(255,
                static_cast<int>(prob_float * 255))));
        } else {
            prob = output->data.uint8[0];
            prob_float = (prob - output->params.zero_point) * output->params.scale;
        }

        // Debug: log probability every 50th inference.
        static uint16_t dbg_frame_count = 0;
        if (++dbg_frame_count >= 50) {
            dbg_frame_count = 0;
            int feat_sum = 0;
            for (int i = 0; i < kFeatureSize; i++) feat_sum += abs(feature[i]);
            ESP_LOGI(TAG, "DBG prob=%u raw_type=%d float=%.4f cutoff=%u feat_sum=%d",
                     (unsigned)prob, output->type, prob_float,
                     (unsigned)probability_cutoff_u8_, feat_sum);
        }
