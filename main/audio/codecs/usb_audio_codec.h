 ...
 * 
 * 支持 ESP32-S3 作为 USB Host，连接 USB 麦克风设备
 * 兼容 UAC1.0 和 UAC2.0 标准
 * 注意：仅支持麦克风输入，不支持扬声器输出
 */
...
public:
     ...
     * @param output_sample_rate 期望的输出采样率（Hz），设为 0 表示禁用输出
     */
...
private:
    ...
    /// USB 音频设备事件回调
    static void usb_event_callback(uac_host_device_handle_t uac_device_handle,
                                   const uac_host_device_event_t event, void* user_data);
