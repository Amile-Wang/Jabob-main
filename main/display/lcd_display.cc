...
#include "board.h"
#include "icon_manager.h"  // 添加图标管理器头文件
#include "gif_manager.h"  // 添加GIF管理器头文件
...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_style_bg_color(emotion_icon_, lvgl_theme->background_color(), 0); // 确保屏幕背景应用主题颜色
    ...
    lv_obj_set_style_border_color(content_, lvgl_theme->border_color(), 0); // Border color for content
...
void LcdDisplay::ApplyEmotionGif(const char* emotion) {
    ...
    static const std::unordered_map<std::string, gif_id_t> emoji_gif_map = {
        ...
        {"neutral", GIF_NATURAL},      
        {"happy", GIF_SPEAK},          
        {"laughing", GIF_NATURAL},    
        {"funny", GIF_NATURAL},          
        {"sad", GIF_CRY_ERROR},              
        {"angry", GIF_CRY_ERROR},          
        {"crying", GIF_CRY_ERROR},        
        {"loving", GIF_NATURAL},        
        {"embarrassed", GIF_1}, 
        {"surprised", GIF_LISTEN},  
        {"shocked", GIF_LISTEN},      
        {"thinking", GIF_LISTEN},    
        {"winking", GIF_NATURAL},      
        {"cool", GIF_COOL},            
